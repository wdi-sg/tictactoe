...
function onResult(result, scores) {
    setTimeout(function(){ // Wait for DOM to get updated
        if (result == 'draw') {
            alert("It's a draw !");
        } else {
            alert(result + " has won");
            updateScores(scores.X, scores.O); // Update scores of the users
        }
        tictactoe.empty(); // What ever the result may be, empty the board
    });
}
...
function restart(grid_size) {
    ...
    // if grid size is mentioned then re-paint the board with new grid size
    if (grid_size) {
...
/*
 * placeholder - HTML Element where board is painted
 * grid_size   - if 3 then 3 X 3 board is painted
 * callback    - callback function to be called upon result
 */
